...
class SnapController(object):
    ...
    def showClient(self, client, multiline=True):
        ...
        pyotherside.send('%s (%s)' %(clientname, groupname))

    def showAllClients(self):
        pyotherside.send('showing clients')
    ...
    def status(self):
        pyotherside.send("Zones:")
        for group in self._snapserver.groups:
            name = getdefault(group.name, '<nameless>')
            pyotherside.send("    [%s] name='%s' stream='%s'" %(group.identifier, group.name, group.stream))

            for id in group.clients:
                client = self._snapserver.client(id)
                pyotherside.send("        [%s] %s" %(client.identifier, client.friendly_name))

        pyotherside.send("Streams:")
        for stream in self._snapserver.streams:
            pyotherside.send("    [%s] %s" %(stream.status, stream.name))
...
if __name__ == '__main__':
    pyotherside.send("calling main")
    main()


pyotherside.send("not calling main")
